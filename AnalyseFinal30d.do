...
graph twoway (fpfit MortsPerHabitant IndexCollecidv, ///
legend(label(1 "Nombre de morts par habitants prédits") label(2 "Morts par habitants (Pays)")) ytitle("{bf:Nombre de morts  / Population (en milliers)}" " ", size(small)) yline(717.283,lcolor(red) lpattern(dash)) xline(60 70 80,lcolor(green) lpattern(dash)) ///
graphregion(fcolor(gs15))) (scatter MortsPerHabitant IndexCollecidv,xtitle("{bf:Indice d'individualisme}", size(small)) ///
ylabel(0(.05)0.4) text(0.25 60 "Seuil: 60", place(w) color(green) box) text(0.28 70 "Seuil: 70", place(w) color(green) box) text(0.32 80 "Seuil: 80", place(w) color(green) box) title("Prediction du nombre de morts par habitants selon l'indice d'invidualisme" "({it:30 jours après le 1000 cas confirmés})",size(medsmall) box bexpand) mlabel(Country) ///
note("{bf:Source:} JHU & Geert Hofstede"))

graph export "30d/PredictionMortsPerHabitants.png", replace

graph twoway (fpfit MortsPerHabitant DemocracyindexEIU, ///
legend(label(1 "Nombre de morts par habitants prédits") label(2 "Morts par habitants (Pays)") ) ytitle("{bf:Nombre de morts  / Population (en milliers)}" " ", size(small)) yline(717.283,lcolor(red) lpattern(dash)) xline(60 70 80,lcolor(green) lpattern(dash)) ///
graphregion(fcolor(gs15))) (scatter MortsPerHabitant DemocracyindexEIU,xtitle("{bf:Democracy Index}") ///
ylabel(0(.05)0.4) text(0.25 60 "Seuil: 60", place(w) color(green) box) text(0.28 70 "Seuil: 70", place(w) color(green) box) text(0.32 80 "Seuil: 80", place(w) color(green) box) title("Prediction du nombre de morts selon le Democracy Index" "({it:30 jours après le 1000 cas confirmés})",size(medsmall) box bexpand) mlabel(Country) ///
note("{bf:Source:} JHU & Geert Hofstede"))
...
reg MortsPerHabitant Demo60 $controls, robust
reg MortsPerHabitant Demo70 $controls, robust
reg MortsPerHabitant Demo80 $controls, robust
